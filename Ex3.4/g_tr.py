...
y = sin(pi*x1)*sin(pi*x2)*sin(pi*x3)*sin(pi*x4)
...
def data_gen_interior(collocations):
    ...
    y_gt = [ldy(d[0],d[1],d[2],d[3]) for d in collocations]
        
    
    f = [ldf(d[0],d[1],d[2],d[3]) for d in collocations]

    return from_seq_to_array([y_gt,f])

def data_gen_bdry(collocations):
    #how to parse the input?
    y_gtbdry = [ldy(d[0],d[1],d[2],d[3]) for d in collocations]
