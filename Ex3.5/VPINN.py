...
bdrydat = torch.tensor(data_gt.generate_data(data_gt.bdry,bdry_col.detach().numpy())).unsqueeze(-1)
print(bdrydat.shape,"!!!")
...
f = torch.tensor(data_gt.generate_data(data_gt.f,quad_dict['quadrature_points'].reshape(-1,2)).reshape(mesh.nrElem,6,1))
#print(f.shape)
...
for epoch in range(max_iter):
    ...
    if epoch%100 == 0:
        ...
        vy,_,_ = validation.validate_vpinn(u)
