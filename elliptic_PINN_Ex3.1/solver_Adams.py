...
def closure():
    ...
    for i,subquad in enumerate(loader):
       ...
       loss,pres,bres = pde.pdeloss(y,ttintx1,ttintx2,f,tbdx1,tbdx2,bdrydat,A,beta,gamma,bw)
       loss.backward()
       optimizer.step()
...
plt.pcolor(ms_x,ms_y,ms_ysol, cmap='jet',norm=mpl.colors.Normalize(vmin=-0.05, vmax=1.05))
h=plt.colorbar(format='%.2f',norm=mpl.colors.Normalize(vmin=-0.05, vmax=1.05))
...
plt.pcolor(ms_x,ms_y,ms_ugt, cmap='jet',norm=mpl.colors.Normalize(vmin=-0.05, vmax=1.05))
h=plt.colorbar(format='%.2f',norm=mpl.colors.Normalize(vmin=-0.05, vmax=1.05))
h.ax.tick_params(labelsize=20)
plt.xticks([])
plt.yticks([])
